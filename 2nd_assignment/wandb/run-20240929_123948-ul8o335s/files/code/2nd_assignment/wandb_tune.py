...
def generate(model, src, max_len, start_symbol, device):
    ...
    with torch.no_grad():
        ...
        for i in range(max_len - 1):
            ...
            if next_token.item() == model.tgt_embedding.num_embeddings - 1:
                break
...
def evaluate(model, dataloader, criterion, device, tgt_vocab, max_len):
    ...
    with torch.no_grad():
        for src_batch, tgt_batch in tqdm(dataloader, desc='Evaluating', leave=False):
            ...
            preds = [generate(model, src.unsqueeze(0), max_len, tgt_vocab.stoi['<sos>'], device) 
                    for src in src_batch]

            # Decode token indices to text
            preds_text = [tgt_vocab.decode(pred) for pred in preds]
            refs_text = [[tgt_vocab.decode(ref[1:])] for ref in tgt_batch.tolist()]  # Skip the start token

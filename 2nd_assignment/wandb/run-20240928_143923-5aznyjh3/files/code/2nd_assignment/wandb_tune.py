...
def _train_model(scratch_location, src_lang, tgt_lang, epochs=10, batch_size=64, max_len=100, learning_rate=0.0001, 
                tokenizer_type='simple', num_layers=6, num_heads=8, model_dim=512, d_ff=2048, dropout=0.1):
    
    wandb.init(project="ANLP_2nd_assignment", config={
        "epochs": epochs,
    ...
    train_loader, dev_loader, _ = get_dataloaders(scratch_location, src_lang, tgt_lang, config.batch_size, config.max_len, tokenizer=tokenizer_type)
    ...
    encoder = Encoder(src_vocab_size, config.model_dim, num_heads=config.num_heads, num_layers=config.num_layers, d_ff=config.d_ff, max_seq_len=config.max_len, dropout=dropout)
    decoder = Decoder(tgt_vocab_size, config.model_dim, num_heads=config.num_heads, num_layers=config.num_layers, d_ff=config.d_ff, max_seq_len=config.max_len, dropout=dropout)

    # Full transformer model
    model = Transformer(encoder, decoder, config.model_dim, src_vocab_size, tgt_vocab_size, config.max_len).to(device)
